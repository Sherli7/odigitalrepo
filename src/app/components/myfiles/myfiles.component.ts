...
export class MyfilesComponent implements OnDestroy {
  ...
  ngOnDestroy() {
    // Vider relativePath dans le localStorage
  ...
  getName(newName: any,nodeType:string) {
    ...
    if (this.parentNameStack.length === 0 || this.parentNameStack[this.parentNameStack.length - 1] !== newName ||
      this.nodeTypeStak.length === 0 || this.nodeTypeStak[this.nodeTypeStak.length - 1] !== nodeType || 
      this.relativePathStack.length === 0 || this.relativePathStack[this.relativePathStack.length - 1] !== newName) {
      this.parentNameStack.push(newName);
      this.currentNodeType=nodeType;
      this.nodeTypeStak.push(nodeType);
      localStorage.setItem('nodeType', nodeType);
      console.log('Polygamie: '+ nodeType);
  ...
  previous(): void {
    if (this.parentStack.length > 0) {
      ...
      this.nodeTypeStak.pop();
      
      // Mettre à jour le chemin dans le localStorage  
      if (this.parentStack.length > 0) {
        const newParent = this.parentStack[this.parentStack.length - 1];
        localStorage.setItem('nodeType', JSON.stringify(this.nodeTypeStak[this.nodeTypeStak.length-1]));
